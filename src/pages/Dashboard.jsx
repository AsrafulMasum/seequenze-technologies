  ...
  return (
    <div className="p-10">
      <div className="flex justify-between items-center">
        ...
        <Link to="/addAProject" className="btn text-white">Add a Project</Link>
      </div>
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
        {cards?.map((card) => (
          <Link to={`/projectDetails/${card?._id}`} key={card?._id} className="card">
            <div className="h-48 rounded-t-xl">
              <img src={card?.image_url} alt="Shoes" className="h-full w-full rounded-t-xl object-cover" />
            </div>
